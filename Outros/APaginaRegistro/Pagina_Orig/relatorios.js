...
function normalizarRegistro(r) {
  const safe = (v, alt) => (v !== undefined && v !== null && String(v).trim() !== "" ? v : alt);
  return {
    ...
    // Possui Infant
    utilizouInvoice:      safe(r.utilizouInvoice ?? r.possuiInfant ?? r.possui_infant ?? r.invoiceUsado, ""),
    // Emissor da Reserva
    empresaInvoice:       safe(r.empresaInvoice ?? r.emissor ?? r.emissorReserva ?? r.emissor_da_reserva, ""),
    // Invoice
    valorInvoice:         safe(r.valorInvoice ?? r.invoice ?? r.invoice_txt, ""),
    // Link de Pagamento (sim/nao)
    realizouPagamento:    safe(r.realizouPagamento ?? r.linkPagamento ?? r.utilizou_link ?? r.utilizou_link_pagamento, ""),
  ...
  try {
    ...
    CAMPOS_FILTRO.forEach(campo => {
      ...
      valoresUnicos.forEach(valor => {
        ...
        option.value = valor;
        option.textContent = campo.id === 'empresaInvoice' ? formatEmpresa(valor) : valor;
    ...
    const labelDe = document.createElement("label"); labelDe.textContent = "De"; labelDe.setAttribute('for','dataInicio');
    const dataInicio = document.createElement("input"); dataInicio.type = "date"; dataInicio.id = "dataInicio";
    const labelAte = document.createElement("label"); labelAte.textContent = "Até"; labelAte.setAttribute('for','dataFim');
    const dataFim = document.createElement("input"); dataFim.type = "date"; dataFim.id = "dataFim";
    const botao = document.createElement("button"); botao.textContent = "Filtrar"; botao.className = 'btn-primary'; botao.onclick = filtrarRelatorio;
    datasWrap.appendChild(labelDe); datasWrap.appendChild(dataInicio); datasWrap.appendChild(labelAte); datasWrap.appendChild(dataFim); datasWrap.appendChild(botao);
...
async function filtrarRelatorio() {
  try {
    ...
    CAMPOS_FILTRO.forEach(c => {
      ...
      if (valor) {
        filtrados = filtrados.filter(r => String(r[c.id] ?? '').toLowerCase() === valor.toLowerCase());
      }
    });
    ...
    CAMPOS_FILTRO.forEach(c => {
      const el = document.getElementById(`filtro_${c.id}`);
      const valor = el ? el.value : "";
      if (valor) ativos.push(`${c.label}: ${c.id === 'empresaInvoice' ? formatEmpresa(valor) : valor}`);
    });
    if (dataInicio || dataFim) {
      let periodo = "Período: ";
...
function exportarExcel() {
  if (registrosFiltrados.length === 0) { alert("Nenhum registro para exportar."); return; }
...
function converterData(dataStr) {
  ...
  const match = dataStr.match(/^(\d{2})\/(\d{2})\/(\d{4})[^\d]*(\d{2}):(\d{2})(?::(\d{2}))?$/);
...
function formatarData(iso) { const [a,m,d] = iso.split('-'); return `${d}/${m}/${a}`; }

// ================== INIT ==================
window.onload = async function() {
  try {
    document.getElementById("resumoFiltro").textContent = "Selecione os filtros desejados e clique em 'Filtrar' para buscar os registros.";
  } catch(_) {}
